...
def get_dep(device, log, target, encoders, capabilites):
    dep = {}
    for cap in log['prev']:
    ...
    for cap in list(h.keys()):
        if set(h[cap]) != set([-1]):
            for i in range(len(cap)):
                if h[cap][i] == -1:
                    h[cap][i] = config[device][(cap[i].split('+')[-1],)][0]
        else:
...
def divide_by_cmd_cap(device, log, capabilites):
    ...
    for cmd in list(cmd_logs.keys()):
        cmd_logs[cmd]['dep'] = get_dep(device, cmd_logs[cmd]['log'], cmd_logs[cmd]['target'], encoders, capabilites)
...
def main(device, save):
    ...
    if save:
        save_data('./smartAID_models/'+device+'_trained', result)
        save_data('./smartAID_models/'+device+'_encoders', encoders)
